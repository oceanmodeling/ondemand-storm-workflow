...
def create_search_tree(longitude, latitude):
    ...
    """
    long_lat = np.column_stack((longitude.T.ravel(),
                                latitude.T.ravel()))
...
def find_nearby_prediction(ds,variable,indices):
    ...
    for obs_point in range(obs_count):
        idx_arr = np.delete(indices[obs_point],
                            np.where(indices[obs_point]==max_prediction_index)[0])   #len is length of surrogate model array
...
def main(args):
    ...
    obs_coordinates = stack_station_coordinates(df_obs_storm.Longitude.values,df_obs_storm.Latitude.values)
    ...
    for threshold in thresholds_m:
        ...
        for source in sources:
            ...
            dist, indices = tree.query(obs_coordinates, k=max_neighbors, distance_upper_bound=max_distance*1e-5)  # 0.01 is equivalent to 1000 m
            prediction_prob = find_nearby_prediction(ds=ds_temp,variable=prediction_variable,indices=indices)
            df_obs_storm[f'{source}_prob'] = prediction_prob
                
            # Plot probabilities at obs. points
            plot_probabilities(df_obs_storm,f'{source}_prob',gdf_countries,
                               f"Probability of {source} exceeding {thresholds_ft[threshold_count]} ft \n {storm_name}, {storm_year}, {leadtime}-hr leadtime",
                               os.path.join(save_dir, f"prob_{source}_above_{thresholds_ft[threshold_count]}ft_{storm_name}_{storm_year}_{leadtime}-hr.png"))

            # Loop through probabilities: calculate hit/miss/... & POD/FAR
            prob_count = -1
            for prob in probabilities:
                prob_count+=1
                hit,miss,false_alarm,correct_neg = calculate_hit_miss(df_obs_storm,obs_attribute,f'{source}_prob',threshold,prob)
    ...
    ds_ROC = xr.Dataset(
        ...
        data_vars=dict(
            ...
            false_alarm=(["threshold","storm","leadtime","source","prob"],false_alarm_arr),
            correct_neg=(["threshold","storm","leadtime","source","prob"],correct_neg_arr),
    ...
    for threshold in thresholds_ft:
        ...
        plt.axline((0.0,0.0), (1.0,1.0), linestyle='--', color='grey', label='random prediction')
        source_count=-1
        for source in sources:
            source_count+=1
            plt.plot(FAR_arr[threshold_count,0,0,source_count,:],
        ...
        plt.title(f'{storm_name}_{storm_year}, {leadtime}-hr leadtime, {threshold} ft threshold')
        plt.savefig(os.path.join(save_dir, f'ROC_{storm_name}_{leadtime}hr_leadtime_{threshold}_ft.png'))
        plt.close()
...
def entry():
    ...
    parser.add_argument(
        "--leadtime", help="OFCL track leadtime hr", type=int)
    
    parser.add_argument(
        "--prob_nc_path", help="path to probabilities.nc", type=str) 
    
    parser.add_argument(
        "--obs_df_path", help="Path to observations dataframe", type=str)
